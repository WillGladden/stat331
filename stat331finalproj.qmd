...
In this plot, we see that the simulated datasets have $R^2$ values between 0.08 and 0.15. This indicates the data simulated under this statistical model is weakly similar to what was observed. On average, our simulated data account for about 11% of the variability in the observed sanitation index.
...
sim_data |> 
  ...
  theme_bw()
```
It seems that when comparing the simulated to observed, we still tend to overpredict at lower sanitation instantces and generally under predict when the observed sanitation is higher or near 100. Next, we can visualize how this relationship behaves over the long runs with many simulated sanitation values under a similar process to before. 
